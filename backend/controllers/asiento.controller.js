...
export const crearAsiento = async (req, res) => {
    ...
    try {
        ...
        // Bloqueo post-cierre (Política A): fecha -> Ejercicio ABIERTO ->
        // Periodo de ese ejercicio ABIERTO. Cualquier falla bloquea.
        const validacionPeriodo = await validarEjercicioAbiertoParaEscritura({
            id_empresa: cabecera.id_empresa, fecha: cabecera.fecha, transaction
        });
...
export const actualizarAsiento = async (req, res) => {
    ...
    try {
        ...
        // A) la fecha ORIGINAL del asiento ya pertenece a un ejercicio o
        //    período cerrado -no importa qué se quiera cambiar, ya no se toca.
        // B) si el body trae una fecha NUEVA, tampoco puede "mover" el
        //    asiento hacia adentro de un ejercicio/período cerrado.
        const validacionOriginal = await validarEjercicioAbiertoParaEscritura({
            id_empresa: asiento.id_empresa, fecha: asiento.fecha, transaction
        });
        ...
        if (req.body.fecha !== undefined && req.body.fecha !== asiento.fecha) {
            const validacionNueva = await validarEjercicioAbiertoParaEscritura({
                id_empresa: asiento.id_empresa, fecha: req.body.fecha, transaction
            });
...
export const eliminarAsiento = async (req, res) => {
    ...
    try {
        ...
        // El ejercicio/período cerrado prevalece incluso sobre un pendiente
        // sin id_compraventa: si su fecha cae dentro de un rango ya
        // cerrado, no se borra, sin excepción.
        const validacionEliminar = await validarEjercicioAbiertoParaEscritura({
            id_empresa: asiento.id_empresa, fecha: asiento.fecha, transaction
        });
        if (!validacionEliminar.valido) {
            await transaction.rollback();
            return res.status(validacionEliminar.status).json({ msg: validacionEliminar.msg });
        }
...
export const procesarAsiento = async (req, res) => {
    ...
    try {
        ...
        // si existiera un pendiente histórico de un ejercicio/período que
        // quedó cerrado por alguna inconsistencia previa, no debe poder
        // procesarse. No se confía en lo validado al crear -se revalida acá.
        const validacionProcesar = await validarEjercicioAbiertoParaEscritura({
            id_empresa: asiento.id_empresa, fecha: asiento.fecha, transaction
        });
        if (!validacionProcesar.valido) {
            await transaction.rollback();
            return res.status(validacionProcesar.status).json({ msg: validacionProcesar.msg });
        }
