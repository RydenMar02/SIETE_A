...
export const getComprasPorEmpresa = async (req, res) => {
...
export const reporteComprasPDF = async (req, res) => {
    ...
    try {
        ...
        const registrosPlanos = registros.map(r => ({
            ...
            proveedor: r.ClienteProveedor?.razon_social || 'Sin proveedor',
        ...
        const outputPath = join(__dirname, '../reports/reporte_compras.pdf');
        await page.pdf({ path: outputPath, format: 'A4', printBackground: true, margin: { top: '20mm', bottom: '20mm', left: '15mm', right: '15mm' } });
        await browser.close();

        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', 'inline; filename=reporte_compras.pdf');
        await registrarMovimiento({
            id_usuario: req.usuario.id_usuario,
            id_empresa: parseInt(id_empresa),
            tipo: 'GENERO_PDF_COMPRAS',
            descripcion: 'Generó el PDF del listado de compras'
        });

        res.sendFile(outputPath);
    } catch (error) {
        console.error(error);
        res.status(500).json({ msg: 'Error al generar PDF de compras' });
    }
};