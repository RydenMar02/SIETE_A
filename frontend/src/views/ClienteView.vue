      ...
      <main class="flex-1 overflow-auto bg-slate-100">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 py-8">
          ...
          <ClienteProveedorModal
            ...
            :itemEditar="clienteSeleccionado"
            @cerrar="cerrarModal"
            @actualizartabla="obtenerClientes"
          />

          <div class="flex items-center justify-between border-b border-gray-200 pb-3 mb-4">
            <h2 class="text-2xl font-semibold text-gray-900">Gestión de Clientes</h2>
          </div>
          ...
          <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200">
                ...
                <tbody>
                  <tr v-for="item in clientesPaginados" :key="item.id_clienteproveedor" class="border-b border-gray-100 odd:bg-white even:bg-slate-100 hover:bg-green-50 transition-colors">
                    ...
                    <td class="px-3 py-2 text-center">
                      <button type="button" class="text-blue-600 hover:text-blue-800" @click="editarCliente(item)">
                        <i class="ti ti-pencil text-lg"></i>
                      </button>
                    </td>
                    <td class="px-3 py-2 text-center">
                      <button type="button" class="text-red-600 hover:text-red-800" @click="eliminarCliente(item)">
                        <i class="ti ti-trash text-lg"></i>
                      </button>
                    </td>
                  </tr>
                  <tr v-if="clientesPaginados.length === 0">
                    <td colspan="11" class="text-center text-gray-400 py-6">No se encontraron resultados</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div class="flex items-center justify-between px-4 py-2 border-t border-gray-100 text-xs text-gray-500">
              <span>{{ inicioRango }} - {{ finRango }} / {{ clientesFiltrados.length }}</span>
...
// ---------- Datos ----------
const clientes = ref<ClienteProveedorDetalle[]>([])

const obtenerClientes = async () => {
  try {
    const { data } = await obtenerClientesProveedores('CLIENTE', seleccion.idEmpresa)
    clientes.value = data.registros ?? []
  } catch (error) {
    manejarError(error, 'No se pudieron cargar los clientes.')
  }
}
...
const clientesFiltrados = computed(() => {
  ...
  return clientes.value.filter((c) => {
    const valor = columnaFiltro.value === 'tipo_contribuyente'
      ? ETIQUETAS_TIPO_CONTRIBUYENTE[c.tipo_contribuyente]
      : c[columnaFiltro.value]
...
const totalPaginas = computed(() => Math.max(1, Math.ceil(clientesFiltrados.value.length / TAMANO_PAGINA)))
const clientesPaginados = computed(() => {
  const inicio = (pagina.value - 1) * TAMANO_PAGINA
  return clientesFiltrados.value.slice(inicio, inicio + TAMANO_PAGINA)
})
const inicioRango = computed(() => (clientesFiltrados.value.length === 0 ? 0 : (pagina.value - 1) * TAMANO_PAGINA + 1))
const finRango = computed(() => Math.min(pagina.value * TAMANO_PAGINA, clientesFiltrados.value.length))
...
const editarCliente = (item: ClienteProveedorDetalle) => {
...
// ---------- Eliminar ----------
const eliminarCliente = async (item: ClienteProveedorDetalle) => {
  const confirmacion = await makeConfirm(
    '¿Está seguro?',
    `Esta acción eliminará al cliente: ${item.razon_social}`
  )
